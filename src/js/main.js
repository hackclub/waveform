...
function draw() {
    ...
    if (waveforms.length > MAX_WAVEFORMS) {
        console.log("reached max waveforms");
    }
    for (const waveform of waveforms) {
        ctx.save();
        ctx.translate(canvas.width / 2, canvas.height / 2);
        ctx.rotate(mostRecentId == waveform.id ?
            0 :
    ...
    document.querySelectorAll('.hover').forEach((element) => {
        element.style.color = `hsl(${c}, 70%, 70%)`;
    });
    document.querySelectorAll('.menu-item').forEach((element) => {
        element.style.borderColor = `hsl(${c}, 70%, 70%)`;
    });
    if (playerState === 'playing') {
...
async function nextSong(increment) {
    if (songIndex + increment < 0)
